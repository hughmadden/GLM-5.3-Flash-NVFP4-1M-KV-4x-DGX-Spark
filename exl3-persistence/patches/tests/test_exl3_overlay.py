...
Env knobs, all defaulting to the in-image / fail-closed value so the Dockerfile
gate is unchanged:

* ``EXL3_SELFCHECK_GPU=0``  — skip the GPU GEMM/MoE checks (build sets this).
* ``GLM53_SELFCHECK_IMPORTS=0`` — skip every check that needs ``import
  torch``/``vllm``; the source-text checks still run. For porting work on a
  host with no vLLM installed.
* ``GLM53_SITE=<dir>`` — the site directory holding the ``vllm`` package that
  the source-text checks read (same variable as
  ``verify/verify_overlay_static.py``).
"""
...
# Target tree. GLM53_SITE matches verify/verify_overlay_static.py: the *site*
# directory (the parent of the ``vllm`` package), defaulting to the in-image
# path so the Dockerfile step needs no change. Point it at a locally overlaid
# vLLM tree to run the source-text checks on a host with no torch/vllm.
...
def _check_dflash2_imports() -> None:
    """Import-level half of the DFlash2 gate (needs torch/vLLM importable)."""
    ...
    print("dflash2 imports OK", flush=True)


def _check_dflash2_sources() -> None:
    """Source-text half of the DFlash2 gate. No torch/vLLM import needed."""
    qwen = (VLLM / "model_executor/models/qwen3_dflash.py").read_text()
    ...
    spec_init = (VLLM / "v1/worker/gpu/spec_decode/__init__.py").read_text()
    assert "DFlash2Speculator" in spec_init
    assert "DFlash2DraftModel" in spec_init
    dflash_utils = (VLLM / "v1/worker/gpu/spec_decode/dflash/utils.py").read_text()
    assert 'draft_kv = "auto"' in dflash_utils
    assert '"fp8_ds_mla"' in dflash_utils

    # PORT NOTES §2 / §6.1: at vLLM 83252ea89 upstream ships DFlash2 natively
    # and both APIs the vendored overlay copies were written to shim, so
    # patch_dflash2.py is capability-gated and deliberately does NOT install
    # them. Assert the gate's preconditions AND that the files in place are
    # upstream's newer ones -- the vendored shims carry none of these markers
    # (`qwen3_dflash2.py` says in its own docstring that LogitsProcessor has no
    # `get_top_k_tokens`; `dflash2_speculator.py` has neither `IS_DRAFTING=True`
    # nor `draft_logits_spec`), so a silent downgrade fails here loudly.
    logits_proc = (VLLM / "model_executor/layers/logits_processor.py").read_text()
    assert "def get_top_k_tokens(" in logits_proc, "shimmed API gone: re-derive patch_dflash2.py"
    gumbel = (VLLM / "v1/worker/gpu/sample/gumbel.py").read_text()
    assert "def gumbel_noised_argmax(" in gumbel, "shimmed API gone: re-derive patch_dflash2.py"
    assert "IS_DRAFTING" in gumbel, "shimmed API gone: re-derive patch_dflash2.py"
    dflash2_model = (VLLM / "model_executor/models/qwen3_dflash2.py").read_text()
    assert "get_top_k_tokens(" in dflash2_model, "vendored DFlash2 shim installed over upstream's newer copy"
    dflash2_spec = (VLLM / "v1/worker/gpu/spec_decode/dflash2/speculator.py").read_text()
    assert "IS_DRAFTING=True" in dflash2_spec, "vendored DFlash2 speculator installed: Gumbel draw downgraded"
    assert "def draft_logits_spec(" in dflash2_spec, "vendored DFlash2 speculator installed: draft_logits_spec lost"
    assert (VLLM / "v1/worker/gpu/spec_decode/dflash2/__init__.py").is_file()

    glm = (VLLM / "models/glm5next/nvidia/model.py").read_text()
    ...
    kv = (VLLM / "v1/core/kv_cache_utils.py").read_text()
    assert "DFLASH2-DRAFTER-GROUP" in kv
    # PORT NOTES §3(a): the glm5 helpers renamed their comprehension variables
    # (k, v, s -> name, spec). Same partition, re-anchored on the whole
    # comprehension so a rename cannot quietly widen what counts as a drafter
    # layer (exact type, so KpoolTailSpec subclasses stay out).
    assert (
        "    draft_specs = {\n"
        "        name: spec\n"
    ...
    # PORT NOTES §3(a): same k/v/s -> name/spec rename as above.
    assert "                spec.block_size != 64 or spec.page_size_padded != mla_page\n" in kv
    # PORT NOTES §4: the port adds one edit here -- the drafter group is
    # annotated is_eagle_group=True, because get_kv_cache_groups() early-returns
    # the glm5 path before _annotate_eagle_groups() runs, which left the
    # offloading connector treating the drafter's volatile tail as stable.
    assert (
...
def main() -> int:
    ...
    # Both knobs default ON, so the Dockerfile gate is unchanged and fail-closed.
    # GLM53_SELFCHECK_IMPORTS=0 is for porting work on a host with no torch/vLLM
    # (paired with GLM53_SITE pointed at an overlaid tree): it drops exactly the
    # checks that need a live import, and the source-text gate below still runs.
    require_imports = os.environ.get("GLM53_SELFCHECK_IMPORTS", "1") != "0"
    if require_imports:
        _check_quant_registry()
        _check_tp_shard()
        _check_ext_arch()
        _check_e2_diag_static()
        _check_dflash2_imports()
    else:
        print(
            "GLM53_SELFCHECK_IMPORTS=0 — skipped every torch/vLLM import check; "
            f"source-text checks still run against {VLLM}",
            flush=True,
        )
    _check_dflash2_sources()
    if require_gpu:
        if not require_imports:
            raise SystemExit(
                "EXL3_SELFCHECK_GPU=1 with GLM53_SELFCHECK_IMPORTS=0 is "
                "incoherent: the GPU checks import vLLM."
            )
        _check_gpu_gemm()
    else:
