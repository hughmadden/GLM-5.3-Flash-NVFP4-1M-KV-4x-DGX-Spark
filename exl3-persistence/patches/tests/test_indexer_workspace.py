...
class _MLAAttentionSpec:
    ...
    def __init__(self, compress_ratio: int):
        ...
        # vLLM 83252ea89 reads MLAAttentionSpec.tokens_per_state (int) instead.
        self.tokens_per_state = compress_ratio
...
def test_fail_closed_on_drift() -> None:
    for old, new in (
        ...
        # vLLM 83252ea89: MLAAttentionSpec.compress_ratio -> .tokens_per_state
        ("self.compress_ratio = self.kv_cache_spec.tokens_per_state",
         "self.compress_ratio = self.kv_cache_spec.kv_tokens_per_state"),
    ):
...
def test_live_container_copy_if_enabled() -> None:
    ...
    if os.environ.get("GLM53_INDEXER_LIVE_SSH") != "1":
        return
    jump = os.environ.get("GLM53_LIVE_SSH_JUMP", "user@192.0.2.20")
    head = os.environ.get("GLM53_LIVE_SSH_HEAD", "192.0.2.21")
