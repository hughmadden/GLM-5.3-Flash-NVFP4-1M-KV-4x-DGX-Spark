...
# PORT 83252ea89: upstream wrapped the KDA quant-config save/restore in
# try/finally (a real control-flow change, not a rename), so the injected
# conditional moves INSIDE the try block. Semantics are unchanged: when the
# active quant config is exl3, leave it in place so the KDA projections reach
# Exl3Config.get_quant_method; the restore is now exception-safe for free.
