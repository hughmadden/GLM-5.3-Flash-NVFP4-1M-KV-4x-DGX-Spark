...
# PORT 83252ea89: upstream inserted
# `spec_token_ids = strip_speculative_padding(spec_token_ids)` before
# validate_tokens() in update_draft_token_ids (a real behavioural
# addition, not a rename). Both the anchor and the replacement now carry
# that call, so the overlay's trim runs on the stripped list.
...
UPD_NEW = """    def update_draft_token_ids(self, draft_token_ids: DraftTokenIds) -> None:
        ...
        for req_id, spec_token_ids in zip(
            ...
            if _ak_structured:
                metadata = request.structured_output_request
                # PORT 83252ea89: upstream now strips the -1 speculative
                # padding before grammar validation. Carried through verbatim;
                # adaptive-k only trims request.spec_token_ids afterwards and
                # pins structured-output requests to the full length anyway
                # (_ak_structured is passed to .apply()).
                spec_token_ids = strip_speculative_padding(spec_token_ids)
                spec_token_ids = metadata.grammar.validate_tokens(spec_token_ids)  # type: ignore[union-attr]
            request.spec_token_ids = spec_token_ids
